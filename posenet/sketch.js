...
function gotPoses(poses) {
  ...
  if (poses.length > 0) {
    ...
    if (state == "collecting") {
      ...
      currentTime = millis();
      values.push([parseInt(millis()), (angle * 180) / PI]);
    }
  }
}
  ...
  if (started) {
    ...
    if (state == "collecting" && currentTime - startTime > 6000) {
      ...
      for (var t = 0; t < 1; t += 0.01) {
        interpolated.push(interpolate(t, 5, values));
      }
      values.forEach((x) => plotvalues.push({ x: x[0], y: x[1] }));
